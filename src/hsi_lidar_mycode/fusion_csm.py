...
class CrossModalSelfAttentionFusion(nn.Module):
    ...
    def forward(self, z_h: torch.Tensor, z_l: torch.Tensor):
        ...
        t_h = z_h.flatten(2).transpose(1, 2)  # [B, N, D]
        t_l = z_l.flatten(2).transpose(1, 2)  # [B, N, D]
        tokens = torch.cat([t_h, t_l], dim=1)  # [B, 2N, D]

        for block in self.blocks:
            tokens = block(tokens)

        t_h_refined, t_l_refined = tokens.split(num_tokens, dim=1)
        z_h_refined = t_h_refined.transpose(1, 2).reshape(bsz, dim, h, w)
