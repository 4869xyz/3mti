...
try:
    ...
    from .losses_stage3_class_cond import total_stage3_hsi_class_cond_loss
...
def _build_optimizer(model, lr: float, weight_decay: float):
    ...
    return torch.optim.AdamW(params, lr=lr, weight_decay=weight_decay), params
...
def train_one_epoch(
    ...
    model.train()
    model.use_hsi_class_cond_train_mode()
    if not args.train_lidar_prior:
        model.lidar_prior_head.eval()
    meter = {
        "loss": 0.0,
        "loss_cls": 0.0,
        "loss_kd": 0.0,
    ...
    pbar = tqdm(loader, desc="Train-S3A-HSI-ClassCond", leave=False)
    for step, batch in enumerate(pbar, start=1):
        hsi = batch["hsi"].to(device, non_blocking=True)
        lidar = batch["lidar"].to(device, non_blocking=True)
        labels = batch["label"].to(device, non_blocking=True)

        outputs = model.forward_train_missing_hsi_class_cond(
            hsi=hsi,
            lidar=lidar,
            labels=labels,
            prior_temperature=args.prior_temperature,
        )
        losses = total_stage3_hsi_class_cond_loss(
            outputs=outputs,
            labels=labels,
        ...
        pbar.set_postfix(
            ...
            kd=f"{scalar_losses['loss_kd']:.4f}",
            proto=f"{scalar_losses['loss_proto']:.4f}",
        )

        if args.log_batch_interval > 0 and step % args.log_batch_interval == 0:
            logger.info(
                "Epoch %03d Batch %04d | loss=%.6f cls=%.6f kd=%.6f proto=%.6f "
                "noise=%.6f recon=%.6f align=%.6f prior=%.6f",
                epoch,
                step,
                scalar_losses["loss"],
                scalar_losses["loss_cls"],
                scalar_losses["loss_kd"],
                scalar_losses["loss_proto"],
                scalar_losses["loss_noise"],
                scalar_losses["loss_recon"],
                scalar_losses["loss_align"],
                scalar_losses["loss_prior"],
            )
...
def evaluate_one_epoch(
    ...
    pbar = tqdm(loader, desc="Eval-S3A-HSI-ClassCond", leave=False)
    for batch in pbar:
        lidar = batch["lidar"].to(device, non_blocking=True)
        labels = batch["label"].to(device, non_blocking=True)
    ...
    metrics["per_class_loss"] = np.where(class_count > 0, per_class_loss, 0.0).astype(np.float32)
...
def save_checkpoint(path: str, model, optimizer, epoch: int, best_metrics: Dict[str, float], args) -> None:
    payload = {
        ...
        "prototype_bank": model.prototype_bank.state_dict(),
...
def build_parser() -> argparse.ArgumentParser:
    cfg = Stage3Config()
    parser = argparse.ArgumentParser(description="Stage3A HSI-priority class-conditional latent compensation")
    ...
    parser.add_argument("--lambda_noise", type=float, default=0.4)
    parser.add_argument("--lambda_recon", type=float, default=0.2)
    parser.add_argument("--lambda_align", type=float, default=0.03)
    parser.add_argument("--lambda_prior", type=float, default=0.3)
    parser.add_argument("--kd_temperature", type=float, default=2.0)
    parser.add_argument("--prior_temperature", type=float, default=2.0)
    parser.add_argument("--prototype_momentum", type=float, default=0.99)
    parser.add_argument("--build_prototypes", type=str2bool, default=True)
    parser.add_argument("--train_lidar_prior", type=str2bool, default=True)
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/stage3_houston_hsi_class_cond")
    return parser
...
def main(args) -> None:
    if args.dataset_name.lower() != "houston":
        raise NotImplementedError(
            f"Current Stage3A implementation supports dataset_name='houston' only, got {args.dataset_name}"
        )
    if not args.stage1_ckpt:
        raise ValueError("Stage3A requires --stage1_ckpt.")
    if not args.stage2_mapper_ckpt:
        raise ValueError("Stage3A requires --stage2_mapper_ckpt.")
    ...
    logger = build_logger(
        ...
        filename="train.log",
        logger_name=f"stage3a_hsi_class_cond_{os.path.abspath(args.output_dir)}",
    )
    logger.info("==== Stage3A HSI-Priority Class-Conditional Training ====")
    ...
    model.prototype_bank.momentum = args.prototype_momentum
    ...
    if args.build_prototypes:
        model.eval()
        model.prototype_bank.build_from_loader(model=model, loader=train_loader, device=device)
        logger.info(
            "Prototype bank built | valid_classes=%d/%d counts=%s",
            int((model.prototype_bank.counts > 0).sum().item()),
            meta["num_classes"],
            model.prototype_bank.counts.detach().cpu().tolist(),
        )
    elif not model.prototype_bank.has_valid_prototypes():
        raise RuntimeError("Prototype bank is empty and --build_prototypes is false.")

    model.set_all_trainable(False)
    model.set_hsi_class_cond_trainable(True)
    if not args.train_lidar_prior:
        for param in model.lidar_prior_head.parameters():
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            ...
            logger.info("  Trainable: %s %s", name, tuple(param.shape))
    logger.info("Trainable parameters after freezing: %d", trainable_count)

    optimizer, trainable_params = _build_optimizer(model=model, lr=args.lr, weight_decay=args.weight_decay)
    best_oa = -1.0
    best_aa = -1.0
    best_kappa = -1.0
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if do_eval:
            ...
            logger.info(
                "Epoch %03d | train_loss=%.6f cls=%.6f kd=%.6f proto=%.6f noise=%.6f recon=%.6f align=%.6f prior=%.6f | "
                "eval_ce=%.6f OA=%.6f AA=%.6f Kappa=%.6f",
                epoch,
                train_loss["loss"],
                train_loss["loss_cls"],
                train_loss["loss_kd"],
                train_loss["loss_proto"],
                train_loss["loss_noise"],
                train_loss["loss_recon"],
                train_loss["loss_align"],
                train_loss["loss_prior"],
            ...
            save_confusion_matrix(
                ...
                tag=f"epoch_{epoch:03d}_hsi_missing_class_cond",
            )
            ...
            if is_better:
                ...
                save_confusion_matrix(
                    conf_mat=metrics["confusion_matrix"],
                    save_dir=confusion_dir,
                    tag="best_hsi_missing_class_cond",
                )
                logger.info(
            ...
            history.append({"epoch": epoch, "evaluated": False, "train_loss": train_loss, "eval_metrics": None})
            logger.info(
                "Epoch %03d | train_loss=%.6f cls=%.6f kd=%.6f proto=%.6f noise=%.6f recon=%.6f align=%.6f prior=%.6f "
