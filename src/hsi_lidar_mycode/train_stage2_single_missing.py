...
try:
    ...
    from .losses_stage2 import total_stage2_single_missing_loss
...
def train_one_epoch(
    ...
    for step, batch in enumerate(pbar, start=1):
        ...
        losses = total_stage2_single_missing_loss(
            outputs=outputs,
            labels=labels,
...
def evaluate_one_epoch(
    ...
    pbar = tqdm(loader, desc=f"Eval-S2-{missing_mode}", leave=False)
    for batch in pbar:
        hsi = batch["hsi"].to(device, non_blocking=True)
        lidar = batch["lidar"].to(device, non_blocking=True)
        labels = batch["label"].to(device, non_blocking=True)
...
def build_parser(default_output_dir: Optional[str] = None) -> argparse.ArgumentParser:
    cfg = Stage2Config()
    parser = argparse.ArgumentParser(description="Stage2 single missing-modality training")
    ...
    parser.add_argument("--freeze_backbone_epochs", type=int, default=cfg.freeze_backbone_epochs)
...
def run_training_for_mode(missing_mode: str, default_output_dir: str, description: str) -> None:
    ...
    if args.freeze_backbone_epochs < 0:
        raise ValueError(f"freeze_backbone_epochs must be >= 0, got {args.freeze_backbone_epochs}")
    ...
    logger = build_logger(
        ...
        filename="train.log",
        logger_name=f"stage2_{missing_mode}_{os.path.abspath(args.output_dir)}",
    )
    ...
    for epoch in range(1, args.epochs + 1):
        if args.freeze_backbone_epochs > 0 and epoch == args.freeze_backbone_epochs + 1:
            ...
            optimizer = _build_optimizer(model=model, lr=args.lr, weight_decay=args.weight_decay)
            logger.info("Backbone unfrozen at epoch %d. Optimizer rebuilt.", epoch)
