...
# 对 HSI 做 min-max 归一化（按波段）
def standardize_hsi(hsi: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    h, w, c = hsi.shape
    flat = hsi.reshape(-1, c)  # [H*W, C]
    vmin = flat.min(axis=0, keepdims=True)
    vmax = flat.max(axis=0, keepdims=True)
    vrange = np.maximum(vmax - vmin, eps)
    out = (flat - vmin) / vrange
    return out.reshape(h, w, c).astype(np.float32)


def standardize_lidar(lidar: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    vmin = float(lidar.min())
    vmax = float(lidar.max())
    vrange = max(vmax - vmin, eps)
    return ((lidar - vmin) / vrange).astype(np.float32)
